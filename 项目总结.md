...
#### 4.4 里程碑逾期规则（deriveMilestoneStatus）

里程碑状态推导入口：`deriveMilestoneStatus(milestone, tasks, today?)`（`src/utils/helpers.ts:213-246`）。先按「是否有关联任务」分流，再各自判定，形成统一的"过期 → 逾期 → 消除"时间闭环。

**一、无关联任务（`linked.length === 0`）**

逾期完全由目标日期 `targetDate` 与时间驱动，不再依赖手工存储状态。

| 条件 | 判定结果 | 说明 |
|---|---|---|
| `targetDate` 存在 且 `< 今天` 且 原状态 = `已完成` | `已完成` | 已完成不被翻成逾期 |
| `targetDate` 存在 且 `< 今天` 且 原状态 ≠ `已完成` | `已逾期` | 过期未完成 → 自动逾期 |
| `targetDate` 存在 且 `≥ 今天` 且 原状态 = `已逾期` | `待开始` | 调回未来，自动消除逾期 |
| `targetDate` 存在 且 `≥ 今天` 且 原状态 ≠ `已逾期` | 保持原手工状态 | 如 `待开始`/`进行中`/`有风险` |
| `targetDate` 为空（无目标日） | 保持原手工状态 | 无法判定过期，回显存库状态 |

**二、有关联任务（`linked.length > 0`）**

逾期由「任务完成情况 + 任务 deadline 是否突破里程碑目标日」共同决定。

前置定义：
- `allDone` = 所有关联任务 `status === 已完成`
- `milestoneDeadline` = `targetDate || 今天`
- `anyOverdue` = 存在某个关联任务 `已完成` 且 `deadline > milestoneDeadline`

| 条件 | 判定结果 | 说明 |
|---|---|---|
| `allDone` 且 `!anyOverdue` | `已完成` | 全完成且无任一任务突破目标日 |
| `allDone` 且 `anyOverdue` | `已逾期` | 全完成但有关联任务 deadline 超出目标日 |
| `targetDate < 今天` 且 `!allDone` | `已逾期` | 目标日已过但任务未全部完成 |
| 存在任务 `进行中` 或 `有风险` | `进行中` | 优先级低于上述逾期判定 |
| 其余情况 | `待开始` | 非逾期、非进行中的初始态 |

**三、两条路径的关键差异**

| 维度 | 无关联任务 | 有关联任务 |
|---|---|---|
| 逾期触发依据 | 里程碑自身 `targetDate` 是否过期 | ① 任务 deadline 突破目标日；② 目标日过期且任务未全部完成 |
| `已完成` 是否会被判逾期 | 否（保持 `已完成`） | 否，但若某完成任务 deadline 超目标日则判 `已逾期`（区分"晚完成"与"逾期"） |
| 手工状态 `已逾期` | 可被时间自动消除（目标日回到未来） | 不读取手工状态，完全由任务推导 |
| `targetDate` 为空 | 回显手工状态 | `milestoneDeadline` 退化为"今天"，仍可判定"晚完成"型逾期 |

**四、共同的时间闭环**

两条路径都遵循同一原则：**目标日过期 → 进入 `已逾期`；目标日调回未来 → 退出 `已逾期`**，形成统一的过期→逾期→消除闭环。差异仅在于"逾期"的判定口径（无关联看自身目标日，有关联看关联任务 deadline 是否突破目标日）。
